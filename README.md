...
- RTX 20 / 30：向游戏改写显卡架构的动作提前到游戏启动时，并改报 RTX 50。使用 Streamline 2.8 的游戏（如《最终幻想 7 重生》）此前在启动阶段就判定「本显卡不支持 DLSS-G」并卸掉帧生成插件（issue #509 / #528）；本版在那之前就装好改写，游戏内的 3X / 4X / 6X 选项也按 RTX 50 放开。尚未在真实 RTX 20 / 30 上确认。
...
- Vulkan 支持在项目当前状态下难以继续：现有的 Vulkan 封装层游戏兼容性太差，也没有足够多的覆盖测试。有需要的用户可以选用社区的 patch 版本。
