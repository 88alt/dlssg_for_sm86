...
| `fg_gate_create_feature` | 每次核心的 `CreateFeature`（最多 16 条）：`feature_id`（DLSS-G = 11）、`status`、`succeeded`、`handle`、`caller`，DLSS-G 还带 `parameters`（`multi_frame_count` / `multi_frame_count_max` / `width` / `height`）。**失败是错误级**，`0xbad0000b`（`NVSDK_NGX_Result_FAIL_OutOfDate`）= 核心拿它在自己初始化时从 NVAPI 读到的显卡架构，去比 DLSS-G 运行库（snippet）导出的 `NVSDK_NGX_GetGPUArchitecture`（= 运行库声明的最低架构）后拒绝。后端把那个导出报成**物理架构**（`turing_host.gpu_architecture_export`，Turing 上 `0x160`；之前报的是 Ada `0x190`，那正是真 Turing 上帧生成打不开的原因）。`SpoofArchToGame` 管的是另一道闸门（游戏自己的），对这一条没有帮助——核心的这次比较发生在 Streamline 初始化里，早于我们被要求加载 snippet。 |
